use crate::{
    ...
    symboltable::{Dimension, ParamList},
    LST, ST,
};
...
impl Tnode {
    pub fn get_type(&self) -> Result<DType, Box<dyn Error>> {
        match self {
            ...
            Tnode::Id {
                ...
                .unwrap()
                .get_type(name)
                .ok_or(Box::<dyn Error>::from("ERROR: Variable does not exist!")),
    ...
    pub fn create_literal(
        ...
        match dtype {
            DType::Data(_) => match dtype.get_primitive() {
                Primitive::Int => {
                    let parse_result = value.parse::<i32>();
                    match parse_result {
                        Ok(_) => return Ok(Tnode::Literal { span, dtype, value }),
                        Err(_) => {
                            return Err(LangParseError(
                                span,
                                format!("ERROR: `{}` is an invalid integer!", value),
                            ))
                        }
                    }
                }
                Primitive::Str => Ok(Tnode::Literal { span, dtype, value }),
                _ => Err(LangParseError(
                    span,
                    format!("ERROR: Invalid datatype `{}` found for constant!", dtype),
                )),
            },
            DType::Pointer(_) => match dtype.get_primitive() {
                Primitive::Int | Primitive::Str => {
                    let parse_result = value.parse::<i32>();
                    match parse_result {
                        Ok(_) => Ok(Tnode::Literal { span, dtype, value }),
                        Err(_) => Err(LangParseError(
                            span,
                            format!("ERROR: `{}` is an invalid pointer!", value),
                        )),
                    }
                }
                _ => Err(LangParseError(
    ...
    pub fn create_id_node(
        ...
        let start_address = ST
            .lock()
            .unwrap()
            .get_address(&varname)
            .ok_or(LangParseError(
                name.span(),
                format!("ERROR: Variable does not exist!"),
            ))?;
        let dtype = ST.lock().unwrap().get_type(&varname).ok_or(LangParseError(
            name.span(),
            format!("ERROR: Variable does not exist!"),
        ))?;
        let size = ST.lock().unwrap().get_dim(&varname).ok_or(LangParseError(
            span,
            format!("ERROR: Array size not specified!"),
        ))?;
        let mut address_type = dtype.clone();
        let mut address = Tnode::Literal {
            span,
            dtype: DType::Pointer(Box::new(address_type.clone())),
            value: format!("{}", start_address),
        };
        match size {
                ...
                match index {
                    Dimension::Array(dim_index) => {
                        ...
                        for i in 0..dim_index.len() {
                            if dim_index[i].get_type().map_err(|_| {
                                LangParseError(
                                    dim_index[i].span(),
                                    format!("ERROR: Invalid type found for array index!"),
                                )
            ...
            Dimension::Unit => match index {
                Dimension::Array(_) => {
                    return Err(LangParseError(
                        span,
                        format!(
                        "ERROR: Array variable cannot be accessed without specifying the index!"
                    ),
                    ))
                }
                Dimension::Unit => {
    ...
    pub fn create_op_node(
        ...
        let lhs_type = lhs
            .get_type()
            .map_err(|_| LangParseError(span, format!("ERROR: Invalid type found!")))?;
            ...
            match lhs.clone() {
                ...
                _ => {
                    return Err(LangParseError(
                        span,
                        format!(
                            "ERROR: `{}` operator can only be used with identifiers!",
                            operation
                        ),
                    ))
                }
        ...
        return match id.clone() {
            Tnode::Id {
                ...
                Err(LangParseError(
                    span,
                    format!(
                        "ERROR: Expected type `{}` but found `{}` for assignment of variable `{}`!",
                        dtype, expr_dtype, name
                    ),
                ))
            }
            Tnode::Op {
                    ...
                    OpType::Add => {
                        if let Tnode::Literal { span: _, dtype, .. } = *lhs {
                            if let DType::Pointer(_) = dtype {
                                return Err(LangParseError(
                                    span,
                                    format!("ERROR: Array cannot be reassigned!"),
                                ));
                            }
                        }
                    }
                ...
                return Err(LangParseError(
                    span,
                    format!(
                        "ERROR: Invalid operation `{}` found in LHS of assignment!",
                        optype
                    ),
                ));
            }
            _ => Err(LangParseError(
                span,
                format!("ERROR: Missing identifier or dereference in lhs for assignment!"),
            )),
        };
    }
        ...
        match id_type {
            ...
            _ => {
                return Err(LangParseError(
                    span,
                    format!(
                        "ERROR: Invalid datatype `{}` for identifier in read()!",
                        id_type
                    ),
                ))
            }
    ...
    pub fn create_write_node(span: Span, expr: Tnode) -> Result<Tnode, LangParseError> {
        let expr_type = expr
            ...
            .unwrap();
        match expr_type {
            DType::Data(Primitive::Int) => Ok(Tnode::Write {
                span,
                expr: Box::new(expr),
            }),
            DType::Data(Primitive::Str) => Ok(Tnode::Write {
                span,
                expr: Box::new(expr),
            }),
            _ => Err(LangParseError(
                span,
                format!(
                    "ERROR: Invalid datatype `{}` for expression in write()!",
                    expr_type
                ),
            )),
        }
    ...
    pub fn create_flow_node(
        span: Span,
        ftype: FlowType,
        bool_exprs: Option<Vec<Tnode>>,
        slists: Option<Vec<Tnode>>,
    ) -> Result<Tnode, LangParseError> {
        match ftype {
            FlowType::Continue => Ok(Tnode::FlowStmt {
                span,
                ftype,
                bool_exprs: None,
                slists: None,
            }),
            FlowType::Break => Ok(Tnode::FlowStmt {
                span,
                ftype,
                bool_exprs: None,
                slists: None,
            ...
            _ => match bool_exprs {
                Some(bool_list) => {
                    let is_bool = bool_list.iter().all(|bool_expr| {
                        bool_expr
                            .get_type()
                            .map_err(|_| LangParseError(span, format!("Invalid type found!")))
                            .unwrap()
                            == DType::Data(Primitive::Bool)
                    });
                    if is_bool {
                        if let Some(stmt_list) = slists {
                            return Ok(Tnode::FlowStmt {
                                span,
                                ftype,
                                bool_exprs: Some(
                                    bool_list.into_iter().map(|b| Box::new(b)).collect(),
                                ),
                    ...
                    Err(LangParseError(
                        span,
                        format!("ERROR: No boolean expression found for {}!", ftype),
                    ))
                }
                None => Err(LangParseError(
                    span,
                    format!("ERROR: No boolean expression found for {}!", ftype),
                )),
            },
    ...
    pub fn create_return_node(span: Span, expr: Tnode) -> Result<Tnode, LangParseError> {
        Ok(Tnode::ReturnStmt {
            span,
            dtype: expr.get_type().map_err(|_| {
                LangParseError(
                    span,
                    format!("ERROR: invalid datatype for return statement"),
                )
            })?,
            expr: Box::new(expr),
        })
...
impl FDef {
    pub fn new(flabel: usize, argc: usize, body: Tnode) -> FDef {
        FDef { flabel, argc, body }
    }
    ...
    pub fn create(
        ...
        body: Tnode,
        span: Span,
    ) -> Result<FDef, LangParseError> {
        if let Tnode::Connector { span, lhs:_, rhs } = body.clone() {

            let rstmt_dtype = rhs
                .get_type()
                .map_err(|_| LangParseError(rhs.span(), format!("ERROR: invalid return type")))?;
            if rstmt_dtype != rtype {
                return Err(LangParseError(
                    rhs.span(),
                    format!(
                        "ERROR: Expected return type of `{}` but found `{}`",
                        rstmt_dtype, rtype
                    ),
                ));
            }
            if fname == "main" {
                return Ok(FDef::new(0, 0, body));
            }
            match ST.lock().unwrap().get_params(&fname) {
                Ok(gst_params) => match gst_params {
                    Some(gst_params) => {
                        if gst_params.len() > params.len() {
                            return Err(LangParseError(span, format!("ERROR: Missing arguments")));
                        } else if gst_params.len() < params.len() {
                            return Err(LangParseError(
                                span,
                                format!("ERROR: Additonal arguments found"),
                            ));
                        }
                        for key in params.keys() {
                            let pdtype = params.get(key).unwrap();
                            match gst_params.get(key) {
                                Some(gdtype) => {
                                    if pdtype == gdtype {
                                        continue;
                                    }
                                    return Err(LangParseError(
                                        span,
                                        format!("ERROR: Datatype mismatch in arguments"),
                                    ));
                                }
                                None => {
                                    return Err(LangParseError(
                                        span,
                                        format!("ERROR: `{}` is not a valid parameter as per declaration of `{}()`", key, fname),
                                    ))
                                }
                            }
                        }
                    ...
                    None => {
                        return Err(LangParseError(
                            span,
                            format!("ERROR:Function declaration missing for `{}()`", fname),
                        ))
                    }
                },
                Err(_) => {
                    return Err(LangParseError(
                        span,
                        format!("ERROR: No declaration found for`{}()`", fname),
                    ))
                }
            }
            for key in params.keys() {
                let pdtype = params.get(key).unwrap();
                if LST.lock().unwrap().contains(key) {
                    return Err(LangParseError(
                        span,
                        format!("ERROR: Parameter declared again in body for `{}()`", fname),
                    ));
                }
                LST.lock().unwrap().append(
                    key.to_string(),
                    pdtype.clone(),
                    Dimension::Unit,
                    None,
            ...
            return match ST.lock().unwrap().get_flabel(&fname) {
                Ok(flabel) => match flabel {
                    Some(flabel) => Ok(FDef::new(flabel, params.len(), body)),
                    None => Err(LangParseError(
                        span,
                        format!("ERROR: `{}` is not a function", fname),
                    )),
                },
                Err(_) => Err(LangParseError(
                    span,
        ...
        return Err(LangParseError(span, format!("ERROR: Misformed body in function!")))
    }

    pub fn create_list(
        prevlist: Option<Vec<FDef>>,
        fdef: FDef,
    ) -> Result<Vec<FDef>, LangParseError> {
        let mut prevl: Vec<FDef> = match prevlist {
            Some(flist) => flist,
            None => vec![],
        };
        prevl.push(fdef);
